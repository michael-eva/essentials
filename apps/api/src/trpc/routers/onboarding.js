...
import { checkOnboardingCompletion, getOnboardingData, } from "../../drizzle/src/db/queries";
export const onboardingRouter = createTRPCRouter({
    ...
    postFitnessBackground: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const { fitnessLevel, exercises, otherExercises, exerciseFrequency, sessionLength, } = input;
        const userId = ctx.userId;
        await insertOnboarding({
            userId,
            fitnessLevel,
    ...
    postHealthConsiderations: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const { injuries, injuriesDetails, recentSurgery, surgeryDetails, chronicConditions, otherHealthConditions, pregnancy, } = input;
        const userId = ctx.userId;
        await insertOnboarding({
            userId,
            injuries,
    ...
    postPilatesExperience: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const { pilatesExperience, pilatesDuration, studioFrequency, sessionPreference, apparatusPreference, customApparatus, } = input;
        const userId = ctx.userId;
        await insertOnboarding({
            userId,
            pilatesExperience,
    ...
    postMotivation: protectedProcedure
        ...
        .mutation(async ({ ctx, input }) => {
        const { motivation, otherMotivation, progressTracking, otherProgressTracking, } = input;
        const userId = ctx.userId;
