...
export async function generateAIResponse(userInput, context) {
...
- Recent Workouts: ${context.recentActivity.recentWorkouts.length > 0
        ? context.recentActivity.recentWorkouts
            .slice(-10)
            .map((workout) => `Name: ${workout.workout?.name ?? "Unnamed"}, Activity Type: ${workout.workout?.activityType ?? "Unknown"}, Would Do Again: ${workout.workoutTracking.wouldDoAgain ? "Yes" : "No"}, Duration: ${workout.workoutTracking.durationHours} hours ${workout.workoutTracking.durationMinutes} mins, Intensity: ${workout.workoutTracking.intensity}`)
            .join("; ")
        : "None yet"}
...
`;
    ...
    const transformedResponse = {
        ...
        workouts: parsedResponse.workouts.map((workout) => ({
            ...workout,
 ...
 */
export async function analyzeProgress(userId, timeRange) {
 ...
 */
export async function generateFollowUpQuestions(userId, context) {
 ...
 */
export async function updateUserProfile(userId, interaction) {
 ...
 */
export async function generateProgressReport(userId, timeRange) {
...
//# sourceMappingURL=personal-trainer.js.map