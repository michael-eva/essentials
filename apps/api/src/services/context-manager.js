import { getWorkoutTracking, getOnboardingData, getActivePlan, getWorkoutById, } from "../drizzle/src/db/queries";
/**
 * Builds a comprehensive context object for AI interactions
 */
export async function buildUserContext(userId, _timeRange = {
 ...
 */
function calculateConsistency(workouts, timeRange) {
    ...
    // Calculate averages
    const weeklyAverage = Array.from(workoutsByWeek.values()).reduce((sum, count) => sum + count, 0) /
        workoutsByWeek.size;
    const monthlyAverage = Array.from(workoutsByMonth.values()).reduce((sum, count) => sum + count, 0) / workoutsByMonth.size;
 ...
 */
function calculateGoalProgress(workouts, onboardingData) {
...
function calculateGeneralProgress(workouts) {
...
function calculateWorkoutVariety(workouts) {
 ...
 */
export async function updateContextWithWorkout(userId, _newWorkout) {
 ...
 */
export async function getContextForInteraction(userId, interactionType) {
