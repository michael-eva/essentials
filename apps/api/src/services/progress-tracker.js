import { getProgressTracking, getLatestProgressTracking, getWorkoutTracking, } from "../drizzle/src/db/queries";
import { insertProgressTracking } from "../drizzle/src/db/mutations";
/**
 * Tracks progress for a specific workout
 */
export async function trackWorkoutProgress(userId, workout) {
    ...
    // Calculate metrics
    const metrics = await calculateWorkoutMetrics(userId, workout, recentWorkouts);
    // Analyze progress
    const { achievements, challenges } = await analyzeWorkoutProgress(workout, metrics, recentWorkouts);
 ...
 */
async function calculateWorkoutMetrics(userId, workout, recentWorkouts) {
    // Calculate duration in minutes
    const duration = (workout.durationHours ?? 0) * 60 + (workout.durationMinutes ?? 0);
 ...
 */
async function analyzeWorkoutProgress(workout, metrics, recentWorkouts) {
    const achievements = [];
    const challenges = [];
 ...
 */
function calculateIntensity(workout) {
    return workout.intensity ?? 5;
}
/**
 * Calculates workout consistency
 */
function calculateConsistency(recentWorkouts) {
 ...
 */
function calculateCompletionRate(recentWorkouts) {
    if (recentWorkouts.length === 0)
        return 1;
 ...
 */
function findWorkoutGaps(workouts) {
 ...
 */
export async function getProgressData(userId, timeRange) {
