// AI Notifications Service - Core business logic for AI notifications
...
/**
 * AI Notifications Service - handles all AI notification generation
 */
export class AiNotificationsService {
  ...
  async generateNotification(
    ...
    const fullSystemContext = this.buildNotificationContext(
      systemPrompt?.prompt ?? "",
      systemPrompt?.name ?? "",
      userContext
    );
    ...
    try {
      ...
      await this.deps.insertAiChatMessages({
        userId,
...
export async function generateAiNotification(
  ...
  const service = new AiNotificationsService(deps);
  return service.generateNotification(userId, userContext);
}
